...
<tr>
	...
	<th width="100%"><?php echo 'Id'; ?></th>
	<th width="100%"><?php echo 'pId'; ?></th>	
	<th></th>
</tr>
<?php
...
$q1 = new DBQuery;
$q1->addTable('task_log', 'tl');
$q1->addWhere('task_log_parent = 1');
$logs1 = (($canView) ? $q1->loadList() : array());

print '123123<br/>';
print $logs1[0]['task_log_id'];
...
// dylan_cuthbert: auto-transation system in-progress, leave these lines
	...
	//$s .= '<form action="?m=tasks&a=view&tab=1&task_id=1" method="post"><input type="submit" class="button" value="' . $AppUI->_('new log2') . '"/></form>';
	
	foreach ($logs1 as $row1) {
		$s .= $row1['task_log_description'].'</br>';
	}
	
	$s .= '</td>';
	
	$s .= '<td>'.$row['task_log_id'].'</td>';
	$s .= '<td>'.$row['task_log_parent'].'</td>';	
...
if (getPermission('tasks', 'edit', $task_id)) {
	$s .= '<input type="submit" class="button" value="' . $AppUI->_('new log1') . '" />';
}
$s .= '</form></td></tr>';
echo $s;
?>
